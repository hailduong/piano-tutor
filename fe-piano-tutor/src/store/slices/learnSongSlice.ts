...
export const loadSheetMusic = createAsyncThunk(
  'learnSong/loadSheetMusic',
  async (songId: string) => {
    // In a real implementation, this would fetch the sheet music from an API
    const response = await fetch(`${process.env.PUBLIC_URL}/songs/${songId}.musicxml`);
    const data = await response.json();
    return { songId, sheetMusic: data };
  }
);
...
const learnSongSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(loadSheetMusic.pending, (state) => {
        // Handle loading state if needed
      })
      .addCase(loadSheetMusic.fulfilled, (state, action) => {
        state.songId = action.payload.songId;
        state.sheetMusic = action.payload.sheetMusic;
      })
      .addCase(loadSheetMusic.rejected, (state, action) => {
